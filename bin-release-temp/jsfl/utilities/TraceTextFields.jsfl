				...
				for(var iii=0; iii<nnn; iii++){
					...
					if(curElementObj.libraryItem == undefined){
						...
						if (curElementObj.elementType == "text") {
							//log("\n-----<change font>-----\n");
							//log("NON LIBRARY ITEM curElementObj:"+curElementObj);
							
							if (curElementObj.getTextAttr("face").indexOf("TitleFont") > -1){
								log(theTimeline.name);
								log("[face]:" + curElementObj.getTextAttr("face") + ", [name]:" + curElementObj.name +", [useDeviceFont]:" + curElementObj.useDeviceFonts + "\n");
	                        }
						}
					}
				}
